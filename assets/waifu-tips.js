...
live2d_settings.NLPAPI               = true;                         //启用腾讯NLP
live2d_settings.eyeProtInfo          = true;                         //启用使用时长提醒
//使用时长提醒需要的变量
var setTime =60; //设定的提醒时间(单位分钟)


// 默认模型
live2d_settings.modelId              = 4;            // 默认模型 ID，可在 F12 控制台找到
...
//看板娘样式设置
live2d_settings.waifuSize            = '300x320';    // 看板娘大小，例如 '280x250', '600x535'
...
// 其他杂项设置
live2d_settings.l2dVersion           = '1.0.7 (1.5.5)';        // 当前版本
live2d_settings.l2dVerDate           = '2020.09.18'; // 版本更新日期
live2d_settings.homePageUrl          = 'auto';    // 主页地址，桌面版建议设置为auto，可选 'auto'(自动), '{URL 网址}'
live2d_settings.aboutPageUrl         = 'https://www.zerolite.cn/';   // 关于页地址, '{URL 网址}'
...
function loadTipsMessage(result) {
    	        ...
    	        $.getJSON('https://v1.hitokoto.cn',function(result){
            	    if (!empty(result.from)) {
					    ...
					    $.each(rate, function (i, items) {
								var rater = items["total"];
						
                        text = text.render({source: result.from, creator: result.creator, rate: rater});}); });
                        window.setTimeout(function() {showMessage(text, 3000, true);}, 5000);
            	    }
	...
	$('.waifu-tool .fui-eyeProtInfo').click(function (){
		console.log(filter);
		if(filter===true){clearInterval(timer);showMessage('定时提醒已关闭！',5000,true);filter = false;}
		else
			    eyeProtfunc();
	});}
	function eyeProtfunc()
	{
		...
		console.log(filter);
		document.getElementById('NLPInfinite').style.display = 'block';
		document.getElementById("NLPInfinite").innerHTML = "<div id=\"NLPx\" style =\"width: 263px; height: 140px; top: 15px; left:36px; font-size: 13px; z-index:5;    border: 1px solid rgba(224, 186, 140, 0.9);border-radius: 12px;background-color: rgba(236, 217, 188, 0.9);box-shadow: 0 3px 15px 2px rgba(191, 158, 118, 0.8);text-overflow: ellipsis;overflow: hidden;position: absolute;animation-delay: 5s;animation-duration: 50s;animation-iteration-count: infinite;animation-name: shake;animation-timing-function: ease-in-out; opacity:0.99;\"><div align=center style=\"top:0px;\"><input type=\"text\" id=\"timeset\" name=\"text\" placeholder=\"\" style=\"border-radius:14px;\" align=\"middle\"; required><p><script> function getval(){document.cookie = \"status=1\"; }</script><button onclick = \"getval()\" width = 100% height = 100px></button></p></div></div>";
		
        $('#timeset').focus();
		showMessage('设备使用时长提醒已开启！设定时间为'+setTime+'分钟。再次单击关闭计时！',5000,true);
		timer = setInterval(function(){showMessage('您已使用'+setTime+'分钟！休息一下，放松放松吧！',50000,true);
		var rand = Math.floor(Math.random()*6);
		var audio= new Audio("./Alert Alarms/alert"+rand+".mp3");//这里的路径写上mp3文件在项目中的绝对路径
        audio.play();//播放
		showMessage('您已使用'+setTime+'分钟！休息一下，放松放松吧！',5000,true);},setTime*60000);
		//filter = false;
		//break;
	}
	//工具栏不隐藏
	if (live2d_settings.alwaysshowToolMenu) document.getElementById('NLP2').style.display = 'block';
}
